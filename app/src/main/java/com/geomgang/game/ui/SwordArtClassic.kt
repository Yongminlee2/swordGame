 ...
 * 초창기(M5) 벡터 검 아트 - 복원판.
 *
 * v1.1에서 픽셀 시트로 바뀌며 물러났지만, **계열이 형태를, 티어가 색과 장식을 정하는**
 * 조립식 그림은 그 나름의 맛이 있어 상점의 계열 미리보기로 돌아왔다.
 * 그림 파일 0개, 어떤 크기로도 선명한 벡터라 미리보기 용도에 정확히 맞는다.
 *
...
fun bladeSpecOf(family: WeaponFamily): BladeSpec = when (family) {
    ...
    // --- v1.2 이후 추가된 계열. M5에는 없었으므로 여기서 새로 정의한다. ---

    WeaponFamily.SCYTHE -> BladeSpec(
        widthRatio = 0.10f, lengthRatio = 0.54f, curve = 0.34f, wave = 0f,
        singleEdge = true, twin = false, motif = BladeMotif.PLAIN, guard = GuardStyle.BAR,
    )

    WeaponFamily.AXE -> BladeSpec(
        widthRatio = 0.32f, lengthRatio = 0.42f, curve = 0f, wave = 0f,
        singleEdge = true, twin = false, motif = BladeMotif.PLAIN, guard = GuardStyle.BAR,
    )

    WeaponFamily.SPEAR -> BladeSpec(
        widthRatio = 0.05f, lengthRatio = 0.72f, curve = 0f, wave = 0f,
        singleEdge = false, twin = false, motif = BladeMotif.PLAIN, guard = GuardStyle.BAR,
    )

    WeaponFamily.SPIRIT -> BladeSpec(
        widthRatio = 0.07f, lengthRatio = 0.60f, curve = 0f, wave = 0.06f,
        singleEdge = false, twin = false, motif = BladeMotif.PLAIN, guard = GuardStyle.WING,
    )

    WeaponFamily.FUSED -> BladeSpec(
        widthRatio = 0.14f, lengthRatio = 0.58f, curve = 0f, wave = 0f,
        singleEdge = false, twin = false, motif = BladeMotif.FULLER, guard = GuardStyle.WING,
    )

    WeaponFamily.VOID -> BladeSpec(
        widthRatio = 0.09f, lengthRatio = 0.62f, curve = 0f, wave = 0f,
        singleEdge = false, twin = false, motif = BladeMotif.THORNS, guard = GuardStyle.BAR,
    )
}
