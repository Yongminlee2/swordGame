...
data class ForgeUiState(
    ...
    /**
     * 지금 든 검이 **부서져도 사라지지 않는다면** 되돌아갈 단계. 아니면 null.
     *
     * 전설검(+21 복귀)과 조합검(+1 복귀)이 여기 해당한다. 결과창에서 처음 알면
     * 갑자기 바닥으로 떨어진 것이 버그로 보이므로, 굴리기 전에 미리 말해 준다.
     */
    val shatterFloor: Int? = null,
    /** 담금질. 무한 구간에서만 채워진다. */
    ...
    /** 전설검을 한 번이라도 도감에 바쳤는지. 켜지면 조각으로 다시 조합할 수 있다. */
