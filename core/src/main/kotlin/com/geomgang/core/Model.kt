...
data class GameState(
    ...
    /**
     * 이 판에서 용검을 벼린 적이 있는지 — **시즌2의 영구 표식**([Unlocks.deepUnlocked]).
     *
     * 「용검을 지금 들고 있는가」로만 재면 팔거나 도감에 바치는 순간 사냥터가 잠기고
     * 강화석이 지갑에서 사라진다. 한 번 넘은 산은 다시 넘게 하지 않는다.
     *
     * 진행도([ProgressState])가 아니라 여기 있는 이유: 모드를 초기화하면 시즌1부터
     * 다시 시작하는 것이 맞다. 시즌은 판에 속한 개념이라 [bestLevel] 과 같은 자리에 산다.
     *
     * 기본값이라 옛 세이브도 그대로 열린다 — 이미 용검을 가진 판은
