...
object RateTable {
     ...
     *
     * v2.3에서 **+10~+20 을 한 번 더 올렸다**(46/22/5% → 50/28/10%). 파괴 구간이
     * +13 에서 +10 으로 내려오면서 그 구간을 지나는 시간이 길어졌고, 실기기에서
     * "여기서 너무 오래 막힌다"는 말이 나왔다. +20 목표가 5% 에서 10% 로 두 배다.
     *
     * **표는 단조 감소여야 한다**(`RateTableTest`). 값은 `BalanceSimulationTest`
     * 구간과 함께 움직인다 — 여기를 만지면 반드시 그것부터 돌린다.
     */
    private val BASE = doubleArrayOf(
        0.00,
        0.95, 0.90, 0.85, 0.80, 0.75,
        0.70, 0.64, 0.58, 0.52, 0.50,
        0.45, 0.40, 0.36, 0.32, 0.28,
        0.24, 0.20, 0.16, 0.13, 0.10,
    )
    ...
    fun destroyChance(targetLevel: Int): Double {
        ...
        //
        // v2.5에서 +16 위를 다시 낮췄다. **+21 이상이 1.00 이었다** — 실패가 곧
        // 파괴라 화면에 91~96% 가 찍혔고, 되돌아가는 자리가 +21(전설)이나 +1(용검)
        // 이라 한 번 미끄러지면 스무 단계가 통째로 날아갔다. 게다가 조합검·전설검은
        // 방지권이 붙지 않아 **막을 방법 자체가 없다.**
        //
        // 이제 무한 구간의 실패도 대부분 **한 단계 하락**이다. 깊이 갈수록 파괴 몫이
        // 커져 긴장은 남는다.
        return when {
            ...
            targetLevel <= 18 -> 0.14
            targetLevel <= MAX_FINITE_LEVEL -> 0.18
            targetLevel <= 30 -> 0.22
            targetLevel <= 40 -> 0.28
            else -> 0.35
        }
