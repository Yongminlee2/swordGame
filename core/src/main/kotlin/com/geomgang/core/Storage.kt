 ...
 * 사냥에서 검이 떨어지는 규칙.
 *
 * **지금은 게임이 이 규칙을 쓰지 않는다.** 사냥이 검까지 뱉으면 강화로 올린 한 자루의
 * 값어치가 흐려지고, 보관함이 저절로 차서 상점에 갈 이유가 사라진다.
 * 사냥은 골드·조각·강화석·펫만 준다.
 *
 * 규칙과 테스트는 남겨 둔다 — 드롭을 되살릴 때 확률 곡선을 처음부터 다시 짜지 않기
 * 위해서다. 지금 조합 재료는 상점의 "가방에 넣기" 와 조각 교환에서 온다.
 */
object SwordDrop {
